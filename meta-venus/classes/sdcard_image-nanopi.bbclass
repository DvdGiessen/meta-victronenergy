...
#            2MiB              40MiB      SDIMG_ROOTFS  SDIMG_ROOTFS  SDIMG_DATA
# <-----------------------> <----------> <-----------> <-----------> <---------->
#  ------------------------ ------------ ------------- ------------- ------------
...
# SD card image name
SDIMG = "${DEPLOY_DIR_IMAGE}/${IMAGE_NAME}.rootfs.nanopi-sdimg"
...
IMAGE_CMD_nanopi-sdimg () {
	...
	# Burn Partitions
	zcat ${DEPLOY_DIR_IMAGE}/venus-boot-image-nanopi.vfat.gz | dd of=${SDIMG} conv=notrunc seek=1 bs=$(expr ${IMAGE_ROOTFS_ALIGNMENT} \* 1024)
...
ROOTFS_POSTPROCESS_COMMAND += " nanopi_generate_sysctl_config ; "

nanopi_generate_sysctl_config() {
	# systemd sysctl config
	test -d ${IMAGE_ROOTFS}${sysconfdir}/sysctl.d && \
		echo "vm.min_free_kbytes = 8192" > ${IMAGE_ROOTFS}${sysconfdir}/sysctl.d/nanopi-vm.conf
